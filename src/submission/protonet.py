...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            ### START CODE HERE ###
            z_support = self._network(images_support)
            z_query = self._network(images_query)

            # Ensure prototypes are computed outside of the gradient calculation
            with torch.no_grad():
                # Calculate prototypes by averaging support features for each class
                prototypes = torch.stack([z_support[labels_support == label].mean(0) for label in torch.unique(labels_support)])

            # Calculate squared Euclidean distance for query set
            distances_query = torch.cdist(z_query, prototypes, p=2).pow(2)
            # Calculate log probabilities from negative distances for the query set
            log_p_y_query = F.log_softmax(-distances_query, dim=1)
            # Calculate loss for the query set based on log probabilities
            loss = F.nll_loss(log_p_y_query, labels_query)
            loss_batch.append(loss)

            # Calculate prediction accuracy for the query set
            y_hat_query = log_p_y_query.argmax(1)
            accuracy_query = (y_hat_query == labels_query).float().mean().item()  # Ensure conversion to Python scalar
            accuracy_query_batch.append(accuracy_query)

            # Optionally, if you need to calculate accuracy for the support set in a similar manner
            # Note: This might not be necessary if your task only requires query set accuracy
            # But including this for completeness and to address potential requirements
            distances_support = torch.cdist(z_support, prototypes, p=2).pow(2)
            log_p_y_support = F.log_softmax(-distances_support, dim=1)
            y_hat_support = log_p_y_support.argmax(1)
            accuracy_support = (y_hat_support == labels_support).float().mean().item()  # Ensure conversion to Python scalar
